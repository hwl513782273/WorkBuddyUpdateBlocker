#!/bin/zsh
# WorkBuddy 更新屏蔽器 v1.6-beta 构建脚本
# 用法: zsh build_v1.6.sh
...
# 4. 打包 DMG
rm -f "WorkBuddy更新屏蔽器_v1.6-beta.dmg"
hdiutil create -volname "WorkBuddy更新屏蔽器" -srcfolder "$APP" -ov -format UDZO "WorkBuddy更新屏蔽器_v1.6-beta.dmg" 2>&1 | tail -2
...
for kw in "com.workbuddy.workbuddy.BundleMigration" "选择更新包备份目录" "自动备份更新包" "禁止下载缓存读写" "禁止解压暂存读写" "trashItem"; do
...
# 9. 交付（只增不删改：v1.6 新文件名，v1.0/1.1/1.2/1.3/1.4/1.5 保留）
mkdir -p "$DST"
cp "WorkBuddy更新屏蔽器_v1.6-beta.dmg" "$DST/"
cp -R "$APP" "$DST/WorkBuddy更新屏蔽器_v1.6-beta.app"
cp main.swift "$DST/main_v1.6.swift"
cp Info.plist "$DST/Info_v1.6.plist"
echo "== 交付完成 =="
ls -la "$DST" | grep -E "v1\.[0-6]"
