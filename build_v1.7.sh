#!/bin/zsh
# WorkBuddy 更新屏蔽器 v1.7-beta 构建脚本（UI 重构：左右分屏两模式）
# 用法: zsh build_v1.7.sh
...
# 2. 编译（stderr 落日志，便于查错；用 $? 判断退出码，规避 zsh PIPESTATUS 差异）
echo "== 编译 v1.7-beta =="
swiftc -O -parse-as-library -framework SwiftUI -framework AppKit -framework Cocoa -framework Combine \
  main.swift -o "$APP/Contents/MacOS/WorkBuddy更新屏蔽器" 2> /tmp/wb_compile_v17.log
if [ $? -ne 0 ]; then
  echo "COMPILE FAILED"; tail -30 /tmp/wb_compile_v17.log; exit 1
fi
...
# 4. 打包 DMG
rm -f "WorkBuddy更新屏蔽器_v1.7-beta.dmg"
hdiutil create -volname "WorkBuddy更新屏蔽器" -srcfolder "$APP" -ov -format UDZO "WorkBuddy更新屏蔽器_v1.7-beta.dmg" 2>&1 | tail -2
...
for kw in ["更改域名模式","禁止下载模式","自动备份更新包","download.codebuddy.cn","com.workbuddy.workbuddy.BundleMigration","trashItem"]:
...
# 9. 交付（只增不删改：v1.7 新文件名，v1.0/1.1/1.2/1.3/1.5/1.6 保留）
mkdir -p "$DST"
cp "WorkBuddy更新屏蔽器_v1.7-beta.dmg" "$DST/"
cp -R "$APP" "$DST/WorkBuddy更新屏蔽器_v1.7-beta.app"
cp main.swift "$DST/main_v1.7.swift"
cp Info.plist "$DST/Info_v1.7.plist"
echo "== 交付完成 =="
ls -la "$DST" | grep -E "v1\.[0-7]"
