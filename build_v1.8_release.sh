#!/bin/zsh
# WorkBuddy 更新屏蔽器 v1.8 正式版构建脚本（去 -beta）
# 用法: zsh build_v1.8_release.sh
...
swiftc -O -parse-as-library -framework SwiftUI -framework AppKit -framework Cocoa -framework Combine \
  main.swift -o "$APP/Contents/MacOS/WorkBuddy更新屏蔽器" 2> /tmp/wb_compile_v18r.log
if [ $? -ne 0 ]; then
  echo "COMPILE FAILED"; tail -30 /tmp/wb_compile_v18r.log; exit 1
fi
...
# 4. 打包 DMG（正式版文件名无 -beta）
rm -f "WorkBuddy更新屏蔽器_v1.8.dmg"
hdiutil create -volname "WorkBuddy更新屏蔽器" -srcfolder "$APP" -ov -format UDZO "WorkBuddy更新屏蔽器_v1.8.dmg" 2>&1 | tail -2
...
# 9. 交付（只增不删改：正式版 v1.8 新文件名，v1.0~v1.8-beta 全套保留）
mkdir -p "$DST"
cp "WorkBuddy更新屏蔽器_v1.8.dmg" "$DST/"
cp -R "$APP" "$DST/WorkBuddy更新屏蔽器_v1.8.app"
cp main.swift "$DST/main_v1.8.swift"
cp Info.plist "$DST/Info_v1.8.plist"
echo "== 交付完成 =="
ls -la "$DST" | grep -E "v1\.8"
